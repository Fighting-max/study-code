...
table1=function(n,times,sigma_eps){
  lasso_true=rep(0,times)
  adalasso_0.5_true=rep(0,times)
  adalasso_1_true=rep(0,times)
  adalasso_2_true=rep(0,times)
  adalasso_cv_true=rep(0,times)
  for (i in 1:times) {
    Y=data_xy_model0(i=i,n=n,sigma_eps=sigma_eps)[[1]]
    X=data_xy_model0(i=i,n=n,sigma_eps=sigma_eps)[[2]]
    #Lasso
    if(lasso_beta_f(X=X,Y=Y)[1]!=0 & lasso_beta_f(X=X,Y=Y)[2]!=0 & lasso_beta_f(X=X,Y=Y)[3]!=0 & lasso_beta_f(X=X,Y=Y)[4]==0){
      lasso_true[i]=1
    }
    
    #adaptlasso,gamma=0.5
    if(adalasso_beta_f(X=X,Y=Y,gamma=0.5)[1]!=0 & adalasso_beta_f(X=X,Y=Y,gamma=0.5)[2]!=0
       &adalasso_beta_f(X=X,Y=Y,gamma=0.5)[3]!=0&adalasso_beta_f(X=X,Y=Y,gamma=0.5)[4]==0){
      adalasso_0.5_true[i]=1
    }

    #adaptlasso,gamma=1
    if(adalasso_beta_f(X=X,Y=Y,gamma=1)[1]!=0&adalasso_beta_f(X=X,Y=Y,gamma=1)[2]!=0&
       adalasso_beta_f(X=X,Y=Y,gamma=1)[3]!=0 &adalasso_beta_f(X=X,Y=Y,gamma=1)[4]==0){
      adalasso_1_true[i]=1
    }
    
    #adaptlasso,gamma=2
    if(adalasso_beta_f(X=X,Y=Y,gamma=2)[1]!=0&adalasso_beta_f(X=X,Y=Y,gamma=2)[2]!=0&
       adalasso_beta_f(X=X,Y=Y,gamma=2)[3]!=0&adalasso_beta_f(X=X,Y=Y,gamma=2)[4]==0){
      adalasso_2_true[i]=1
    }
    ...
    if(adalasso_beta_f(X=X,Y=Y,gamma=gamma)[1]!=0&adalasso_beta_f(X=X,Y=Y,gamma=gamma)[2]!=0&
       adalasso_beta_f(X=X,Y=Y,gamma=gamma)[3]!=0&adalasso_beta_f(X=X,Y=Y,gamma=gamma)[4]==0){
      adalasso_cv_true[i]=1
    }
  }
  result=list(lasso=sum(lasso_true)/times,adalasso_0.5=sum(adalasso_0.5_true)/times,
              adalasso_1=sum(adalasso_1_true)/times,adalasso_2=sum(adalasso_2_true)/times,
              adalasso_cv=sum(adalasso_cv_true)/times)
  return(result)
}
